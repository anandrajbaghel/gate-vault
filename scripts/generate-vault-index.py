...
### 2. The GitHub Action (`.github/workflows/generate-vault-index.yml`)
I added `paths-ignore` so the action doesn't get stuck in an infinite loop, and swapped `git diff` for a cleaner check on staged files.
...
on:
  push:
    branches:
      - main
    paths-ignore:
      - 'vault-index.json' # Prevents infinite loop from its own commits
...
jobs:
  generate-index:
    ...
    steps:
      ...
      - name: Run Generator
        run: python scripts/generate-vault-index.py
      ...
      - name: Commit Generated Index
        run: |
          ...
          git add vault-index.json
          
          # Check if there are actually changes to commit
          if git diff --staged --quiet; then
            echo "No changes in vault-index.json."
            exit 0
          fi

          git commit -m "chore: auto-generate vault-index.json"
          git push
