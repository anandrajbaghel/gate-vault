...
> [!examtip]
> 
> ==Be careful with the quadrant for $\phi$. If $x$ is negative, the calculator might give the wrong angle. Adjust by $\pm 180^\circ$.==
> 
...
---
#### Calculus Operations on Phasors
#phasor/calculus

When transforming from Time Domain to Frequency Domain ($t \to j\omega$):
Let $v(t) = V_m \cos(\omega t + \phi) \leftrightarrow \mathbf{V} = V_m \angle \phi$.

1.  **Time Derivative ($\frac{d}{dt}$):**
    Corresponds to multiplication by $j\omega$ (or $\omega \angle 90^\circ$).
    $$\frac{d v(t)}{dt} \leftrightarrow j\omega \mathbf{V}$$

2.  **Time Integral ($\int dt$):**
    Corresponds to division by $j\omega$.
    $$\int v(t) dt \leftrightarrow \frac{\mathbf{V}}{j\omega} = \frac{\mathbf{V}}{\omega} \angle -90^\circ$$

> [!examtip] GATE Calculator Tips
> 1.  **Mode:** Always ensure your calculator is in **Complex Mode** (usually `CMPLX`).
> 2.  **Degrees vs Radians:** Ensure the angle setting matches the problem data. Most AC circuit problems use Degrees.
> 3.  **Direct Conversion:** Learn the shortcut keys to convert `Rect->Polar` and `Polar->Rect` directly without manually using the formulas.
>	* *Example:* On Casio fx-991, shift + 2 (CMPLX) gives options for conversion.
> 4.  **Magnitude Shortcut:** `Abs(z)` function gives magnitude $r$. `arg(z)` gives angle $\phi$.
