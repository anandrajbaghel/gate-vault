...
```mermaid
mindmap
  root((Euler's Formulae for Fourier Coefficients))
    Basis: Orthogonality
      (Sinusoids are orthogonal over a period T)
      ∫ sin(mω₀t)cos(nω₀t) dt = 0
      ∫ cos(mω₀t)cos(nω₀t) dt = 0, (m≠n)
      ∫ sin(mω₀t)sin(nω₀t) dt = 0, (m≠n)
    The Coefficients
      a₀ (DC Component)
        (Average value of the function)
      aₙ (Cosine Coefficients)
        (Measures even symmetry content)
      bₙ (Sine Coefficients)
        (Measures odd symmetry content)
    The Formulae
      a₀ = (1/T) ∫ f(t) dt
    ...
    Exponential Form
      ...
      (Derived similarly using orthogonality of e^(jnω₀t))
```
