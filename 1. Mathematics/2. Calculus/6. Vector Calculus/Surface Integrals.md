...
```mermaid
mindmap
  root((Surface Integrals))
    Definition
      Integral over a surface S in 3D space
    Types
      Scalar Field (∬ f dS)
        w.r.t. surface area
        Interpretation: Mass of a thin shell, Total charge
      Vector Field (∬ F ⋅ dS)
        w.r.t. surface area vector (Flux)
        Interpretation: Fluid flow rate, Electric/Magnetic Flux
    Evaluation Procedure
      1. Parameterize the surface S: r(u,v)
      2. Find the normal vector: rᵤ × rᵥ
      3. Substitute for x, y, z
      4. Substitute differentials (dS or d**S**)
      5. Integrate over the parameter domain in the uv-plane
    Key Theorems
      Divergence Theorem (Gauss's Theorem)
        Relates surface integral (flux) to triple integral (divergence)
        ∯ F ⋅ dS = ∭ div(F) dV
      Stokes' Theorem
        Relates surface integral (curl) to line integral (circulation)
        ∬ (∇×F) ⋅ dS = ∮ F ⋅ dr
```
