...
```mermaid
mindmap
  root((Gauss's Divergence Theorem))
    Core Idea
      Relates a closed surface integral (flux) to a triple integral (of the divergence)
    The Formula
      ∯ F ⋅ dS = ∭ (∇⋅F) dV
    Intuition
      Net Outflow from a Volume = Sum of all Sources/Sinks Inside
    Requirements
      E: A solid region
      S: The closed boundary surface of E
      Orientation: Positive (outward normal)
      F: Vector field with continuous partials
    Relationship to Other Theorems
      Generalization of Green's Theorem (Flux Form)
      A fundamental theorem of calculus
    Applications
      Maxwell's Equations (Gauss's Law for E-field)
      Simplifying difficult flux integrals
...
> The **Divergence Theorem**, also known as Gauss's Theorem, is a cornerstone of vector calculus. It establishes a profound relationship between the total **flux** of a vector field through a closed surface and the [[Gradient, Divergence, and Curl|divergence]] of the field within the volume enclosed by that surface. Intuitively, it states that the net outward flow from a region is equal to the sum of all the sources and sinks inside it.
