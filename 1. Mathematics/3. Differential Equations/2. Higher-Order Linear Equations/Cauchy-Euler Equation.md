...
```mermaid
mindmap
  root((Cauchy-Euler Equation))
    Form
      ax²y'' + bxy' + cy = R(x)
      (Power of x matches order of derivative)
    Solution Strategy
      Transform to a constant-coefficient ODE
    Method 1: Substitution
      x = eᵗ  (or t = ln x)
      xy' → Dy
      x²y'' → D(D-1)y
      (where D = d/dt)
      Result: [aD(D-1) + bD + c]y = R(eᵗ)
    Method 2: Trial Solution
      (For Homogeneous Case)
      Assume y = xᵐ
      Derive Auxiliary Equation
    Auxiliary Equation
      am(m-1) + bm + c = 0
    Solution Forms (based on roots of Aux. Eq.)
      Real, Distinct (m₁, m₂)
        y = c₁xᵐ¹ + c₂xᵐ²
      Real, Repeated (m)
        y = (c₁ + c₂ln(x))xᵐ
      Complex (α ± jβ)
        y = xᵅ[c₁cos(βln(x)) + c₂sin(βln(x))]
```
