...
aliases:
  ...
  - Particular Integral
subject: "[[Mathematics]]"
parent: "[[Calculus - Second-Order Linear ODEs]]"
confidence: 10
---

```mermaid
mindmap
  root((Linear Non-Homogeneous ODEs))
    Standard Form
      ay'' + by' + cy = R(x)
      (R(x) is the forcing function)
    General Solution
      y(x) = y_c(x) + y_p(x)
      y_c (Complementary Function)
        - Solution to homogeneous part
        - Natural / Transient Response
      y_p (Particular Integral)
        - Any solution to the full equation
        - Forced / Steady-State Response
    Methods to Find y_p
      Method of Undetermined Coefficients
        - "Guessing" based on the form of R(x)
        - Works for Polynomials, Exponentials, Sines/Cosines
        - Modification Rule is critical
      Method of Variation of Parameters
        - More general, formula-based
        - Uses Wronskian
    Applications
      - RLC Circuits with an active source (V(t), I(t))
