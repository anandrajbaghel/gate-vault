...
```mermaid
mindmap
  root((Method of Separation of Variables))
    Core Idea
      Transform a PDE into a set of simpler ODEs
    The Assumption
      The solution is a product of functions, each of a single variable
      e.g., u(x,t) = X(x)T(t)
    The Procedure
      1. Assume product form
      2. Substitute into PDE
      3. Separate variables algebraically
      4. Set both sides equal to a separation constant (λ)
      5. Solve the resulting ODEs
      6. Apply Boundary Conditions (BCs) to find λ (Eigenvalue Problem)
      7. Apply Principle of Superposition
      8. Apply Initial Conditions (ICs) using Fourier Series
    Applicability
      - Linear Homogeneous PDEs
      - Simple coordinate systems (Cartesian, Cylindrical, etc.)
    Key Applications (EE)
      - The Wave Equation (Transmission Lines)
      - The Heat Equation (Heat Dissipation)
      - Laplace's Equation (Electrostatics)
```
