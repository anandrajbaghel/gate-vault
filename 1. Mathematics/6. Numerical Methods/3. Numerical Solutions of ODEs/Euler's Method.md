...
mindmap
  root((Euler's Method))
    Core Idea
      - Approximate the solution to an ODE by taking small, sequential steps along the tangent line.
    The Problem
      - Solve a first-order Initial Value Problem (IVP)
      - y' = f(x,y) with y(x₀) = y₀
    The Formula
      - yᵢ₊₁ = yᵢ + h * f(xᵢ, yᵢ)
      - "New Value = Old Value + Step Size * Slope"
    Error Analysis
      - Local Error (per step) ∝ h²
      - Global Error (accumulated) ∝ h
      - **First-Order Method**
    Properties
      - Simple to implement
      - Not very accurate
