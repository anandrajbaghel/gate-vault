...
aliases:
  ...
  - RK2
subject: "[[Mathematics]]"
parent: "[[Numerical Methods - Solving ODEs]]"
confidence: 10
---

```mermaid
mindmap
  root((Runge-Kutta (RK) Methods))
    Core Idea
      - Improve upon Euler's method by using a weighted average of slopes over an interval.
      - (Instead of just the slope at the beginning)
    General Form
      - yᵢ₊₁ = yᵢ + h * (weighted average of slopes)
    Common Types
      - RK2 (Second-Order)
        - Heun's Method
        - Midpoint Method
      - **RK4 (Fourth-Order)**
        - The "classic" and most common RK method
    RK4 Method (The Workhorse)
      - "New Value = Old Value + (h/6)*(k₁ + 2k₂ + 2k₃ + k₄)"
      - k₁: Slope at the beginning
      - k₂, k₃: Slopes at the midpoint
      - k₄: Slope at the end
    Properties
      - Self-starting
      - Do not require calculating derivatives of f(x,y)
      - High accuracy for a given step size (RK4 error ∝ h⁴)
      - The standard for solving ODEs numerically
```
