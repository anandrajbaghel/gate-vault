...
```mermaid
mindmap
  root((Span of a Set of Vectors))
    Definition
      The set of all possible linear combinations of the vectors
    How to Test if a Vector is in the Span
      Set up vector equation: c₁v₁ + ... + cₖvₖ = b
      Convert to matrix system: Ax = b
      Check for Consistency (existence of a solution)
    Geometric Interpretation
      Span of one vector → Line
      Span of two non-collinear vectors → Plane
      Span of three non-coplanar vectors → 3D Space
    Key Properties
      The span is always a subspace
      Spanning Set for a Vector Space (Span(S) = V)
      Relation to Column Space C(A)
```
