...
```mermaid
mindmap
  root((Gram-Schmidt Process))
    Goal
      Convert a basis into an orthonormal basis
    Input
      A set of linearly independent vectors {v₁, v₂, ..., vₙ}
    Output
      An orthonormal set of vectors {u₁, u₂, ..., uₙ}
    Core Idea
      Orthogonal Projection
      Iteratively remove components parallel to previous vectors
    The Process
    ...
    Application
      ...
      Finding orthonormal bases for function spaces
```
