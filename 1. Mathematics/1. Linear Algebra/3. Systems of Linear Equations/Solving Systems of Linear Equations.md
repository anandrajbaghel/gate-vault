...
*   **Non-Trivial (Infinite) Solutions**: The system has non-trivial solutions (in addition to the trivial one) if $\text{rank}(A) < n$. For a square matrix, this is equivalent to $\det(A) = 0$. This condition is fundamental for finding [[Linear Algebra - Eigenvalues and Eigenvectors|eigenvectors]].
...
[[Transpose and Inverse of a Matrix|Matrix Inverse]]
