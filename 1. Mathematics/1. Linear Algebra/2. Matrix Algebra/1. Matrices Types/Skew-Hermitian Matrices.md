...
```mermaid
mindmap
  root((Skew-Hermitian Matrices))
    Definition
      A = -Aᴴ (Conjugate Transpose)
      aᵢⱼ = -conj(aⱼᵢ)
      Diagonal: Zero or Purely Imaginary
    Properties
      Eigenvalues are Zero or Purely Imaginary
      xᴴAx is Zero or Purely Imaginary
    Matrix Decomposition
      Any matrix = Hermitian + Skew-Hermitian
    Connection to Unitary Matrices
      The generator of rotations
      exp(i * Hermitian) = Unitary
      exp(Skew-Hermitian) = Unitary
    Applications
      Quantum Mechanics (Lie Algebra of SU(n))
      Rotation Generators
```
