...
parent: "[[Analog & Digital Electronics]]"
modified: 2026-08-04T09:17:45
---
### Small Signal Analysis
#small-signal-analysis #linearization #ac-analysis

> Small-signal analysis is a fundamental technique used to approximate the behavior of non-linear electronic circuits with linear equations. It is valid for small AC signals superimposed on a large DC bias. This method allows us to use [[Linearity in Electric Circuits|linear circuit]] analysis tools (like [[Kirchhoff's Laws#Kirchhoff's Current Law (KCL)|KCL]], [[Kirchhoff's Laws#Kirchhoff's Voltage Law (KVL)|KVL]], [[Thevenin's theorem]]) to determine the AC characteristics of a circuit, such as gain and impedance.
...
2. **Small-Signal Modeling**:
    * Replace the non-linear device with its corresponding linear small-signal model (e.g., a BJT is replaced by its hybrid-$\pi$ model).
    * The parameters of this model (like $g_m$, $r_\pi$) depend on the Q-point calculated in the DC analysis.
3. **AC Analysis**:
    * Analyze the resulting linear circuit to find AC quantities like voltage gain, current gain, input impedance, and output impedance.
...
This is the most common model used for BJT amplifier analysis. The key parameters are calculated at the Q-point.
...
#mosfet-model
Similar to the BJT, the MOSFET model parameters depend on the DC operating point (assuming operation in the saturation region).
...
* **Output Resistance ($r_o$ or $r_{ds}$)**: Accounts for channel-length modulation.
    ...
    where $\lambda$ is the channel-length modulation parameter and $V_A = 1/\lambda$ is the Early voltage for the MOSFET.
...
[[MOSFET]]
[[Diodes]]
[[Control System]] (The concept of linearization is central to analyzing non-linear systems)